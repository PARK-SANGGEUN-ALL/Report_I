...
export async function parseAll(b64) {
  ...
  return parseAllFromExtracted(text, pages);
}

// 🆕 나이스 대국민서비스 등에서 받은 HTML(.htm/.html) 학생부 파일 파싱
// PDF와 달리 이미 구조화된 <table> 태그를 가지고 있어 오히려 더 안정적으로 파싱 가능.
export async function parseAllFromHtml(htmlString) {
  const { text, pages } = extractHtmlText(htmlString);
  return parseAllFromExtracted(text, pages);
}

export function extractHtmlText(htmlString) {
  const doc = new DOMParser().parseFromString(htmlString, "text/html");
  // 스크립트/스타일 제거 — 텍스트 추출에 방해되는 내용 제거
  doc.querySelectorAll("script,style,noscript").forEach(el => el.remove());

  const lines = [];
  const seenTables = new Set();

  // 모든 표(<table>)를 순서대로 순회하며 행(row) 단위로 텍스트 추출
  // 셀은 tab으로 구분(파서의 나머지 로직이 PDF 열 구분자와 동일하게 처리하도록)
  const walk = (node) => {
    if (!node) return;
    if (node.nodeType === 1) { // ELEMENT_NODE
      const tag = node.tagName?.toLowerCase();
      if (tag === "table" && !seenTables.has(node)) {
        seenTables.add(node);
        ...
        return; // 표 내부는 이미 처리했으므로 재귀 중단
      }
      // 학년 구분 헤더 등으로 자주 쓰이는 블록 요소는 줄바꿈으로 취급
      if (["div","p","br","h1","h2","h3","h4","li","tr"].includes(tag)) {
        const ownText = Array.from(node.childNodes)
          .filter(n => n.nodeType === 3)
          .map(n => n.textContent.trim())
          .filter(Boolean)
          .join(" ");
        if (ownText) lines.push(ownText.replace(/\s+/g, " ").trim());
      }
      Array.from(node.childNodes).forEach(walk);
    } else if (node.nodeType === 3) { // TEXT_NODE (표/블록 요소 밖의 순수 텍스트)
      const t = node.textContent.replace(/\s+/g, " ").trim();
      if (t && node.parentElement && !["SCRIPT","STYLE"].includes(node.parentElement.tagName)) {
        // div/p 등에서 이미 캡처되지 않은 최상위 텍스트만 보강 캡처(중복 최소화 best-effort)
      }
    }
  };
  walk(doc.body);
  ...
  const text = cleanLines.map((l, i) => l).join("\n");
  return { text, pages: [cleanLines] };
}

async function parseAllFromExtracted(text, pages) {
