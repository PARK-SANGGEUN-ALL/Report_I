...
export function estimateProbability(diff, arts=false) {
  ...
  if (diff <= -1.0) return { label: "안정", min: 65, max: 80, mid: 72 };
  if (diff <= -0.5) return { label: "적정", min: 45, max: 60, mid: 52 };
  if (diff <= 0.0)  return { label: "소신", min: 28, max: 42, mid: 35 };
  if (diff <= 0.5)  return { label: "도전", min: 14, max: 26, mid: 20 };
  if (diff <= 1.0)  return { label: "상향", min: 6, max: 14, mid: 10 };
  return { label: "매우 상향(희박)", min: 1, max: 6, mid: 3 };
}
...
export function compareAllUnivs(grade9, major, is5=false, factors=null) {
  ...
  return UNIV_TIERS.map(tier => ({
    ...
    univs: tier.univs.map(u => {
      ...
      let verdict, vColor;
      // 🆕 보수적 판정 v2 — 실제 학과별 입결 편차(최대 1~2등급)를 반영해 기존보다 더 엄격하게.
      // "안정"·"적정" 도달 기준을 좁혀, 평균 컷만 보고 낙관적으로 판단하지 않도록 함.
      if (diff <= -1.0) { verdict = "안정"; vColor = "#16a34a"; }
      else if (diff <= -0.5) { verdict = "적정"; vColor = "#0c8599"; }
      else if (diff <= 0.0) { verdict = "소신"; vColor = "#d97706"; }
      else if (diff <= 0.5) { verdict = "도전"; vColor = "#dc2626"; }
      else if (diff <= 1.0) { verdict = "상향(어려움)"; vColor = "#991b1b"; }
