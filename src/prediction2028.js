...
export function compareAllUnivs(grade9, major, is5=false, factors=null) {
  ...
  return UNIV_TIERS.map(tier => ({
    ...
    univs: tier.univs.map(u => {
      ...
      let verdict, vColor;
      // 보수적 판정 — 한 단계씩 더 빡빡하게
      if (diff <= -0.8) { verdict = "안정"; vColor = "#16a34a"; }
      else if (diff <= -0.3) { verdict = "적정"; vColor = "#0c8599"; }
      else if (diff <= 0.2) { verdict = "소신"; vColor = "#d97706"; }
      else if (diff <= 0.8) { verdict = "도전"; vColor = "#dc2626"; }
      else { verdict = "상향(어려움)"; vColor = "#991b1b"; }
      return { ...u, cutShow, diff: +diff.toFixed(2), verdict, vColor, penaltyNote: penaltyNote.length?penaltyNote:null };
    })
  }));
}
