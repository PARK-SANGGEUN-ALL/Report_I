...
export const UNIV_TIERS = [
  {
    ...
    univs: [
      {name:"서울대", region:"서울", cut:1.5},
      {name:"연세대", region:"서울", cut:1.7},
      {name:"고려대", region:"서울", cut:1.7},
      {name:"KAIST", region:"대전", cut:1.6},
      {name:"포항공대(POSTECH)", region:"경북", cut:1.6},
      {name:"의예과(전국)", region:"전국", cut:1.2},
      {name:"교대(전국)", region:"전국", cut:1.8},
      {name:"사관학교(전국)", region:"전국", cut:2.2},
    ]
  },
  {
    ...
    desc: "서성한·중경외시 + 이화여대",
    univs: [
      {name:"성균관대", region:"서울", cut:1.9},
      {name:"서강대", region:"서울", cut:2.0},
      {name:"한양대", region:"서울", cut:1.9},
      {name:"중앙대", region:"서울", cut:2.1},
      {name:"경희대", region:"서울", cut:2.1},
      {name:"한국외대", region:"서울", cut:2.2},
      {name:"서울시립대", region:"서울", cut:2.1},
      {name:"이화여대", region:"서울", cut:2.0},
    ]
  },
  {
    ...
    univs: [
      {name:"건국대", region:"서울", cut:2.4},
      {name:"동국대", region:"서울", cut:2.4},
      {name:"홍익대", region:"서울", cut:2.5},
      {name:"국민대", region:"서울", cut:2.7},
      {name:"숭실대", region:"서울", cut:2.7},
      {name:"세종대", region:"서울", cut:2.7},
      {name:"단국대", region:"경기", cut:2.8},
      {name:"서울과기대", region:"서울", cut:2.5},
    ]
  },
  {
    ...
    univs: [
      {name:"광운대", region:"서울", cut:2.9},
      {name:"명지대", region:"서울", cut:3.2},
      {name:"상명대", region:"서울", cut:3.3},
      {name:"가천대", region:"경기", cut:3.0},
      {name:"인하대", region:"인천", cut:2.7},
      {name:"아주대", region:"경기", cut:2.6},
      {name:"경기대", region:"경기", cut:3.4},
    ]
  },
  {
    ...
    univs: [
      {name:"부산대", region:"부산", cut:2.5},
      {name:"경북대", region:"대구", cut:2.6},
      {name:"전남대", region:"광주", cut:2.8},
      {name:"전북대", region:"전북", cut:2.9},
      {name:"충남대", region:"대전", cut:2.8},
      {name:"충북대", region:"충북", cut:3.0},
      {name:"강원대", region:"강원", cut:3.2},
      {name:"경상국립대", region:"경남", cut:3.1},
      {name:"제주대", region:"제주", cut:3.3},
    ]
  },
];

// 학생 평균등급(9등급 환산) 대비 각 대학 유불리 판정
export function compareAllUnivs(grade9) {
  const g = parseFloat(grade9);
  if (isNaN(g)) return null;
  return UNIV_TIERS.map(tier => ({
    ...
    univs: tier.univs.map(u => {
      const diff = g - u.cut;
      let verdict, vColor;
      if (diff <= -0.4) { verdict = "안정"; vColor = "#16a34a"; }
      else if (diff <= 0.1) { verdict = "적정"; vColor = "#0c8599"; }
      else if (diff <= 0.6) { verdict = "소신"; vColor = "#d97706"; }
      else { verdict = "도전"; vColor = "#dc2626"; }
      return { ...u, diff: +diff.toFixed(2), verdict, vColor };
    })
  }));
}
